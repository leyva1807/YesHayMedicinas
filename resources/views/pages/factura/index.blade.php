                ...
                <div class="card-body">
                    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-12">
                        ...
                        <div class="lg:col-span-2 ltr:lg:text-right rtl:lg:text-left xl:col-span-2 xl:col-start-11">
                            <a href="{{ route('factura.create') }}" type="button"
                ...
                <div class="!pt-1 card-body">
                    <div class="overflow-x-auto">
                        <table class="w-full whitespace-nowrap" id="productTable">
                            ...
                            @foreach ($facturaCompras as $facturaCompras)
                                <tbody class="list">
                                    <tr>
                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500">
                                            <a href="#!"
                                                class="transition-all duration-150 ease-linear ID Factura text-custom-500 hover:text-custom-600">{{ $factura->id }}</a>
                                        </td>
                                        <td
                                            class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 numero_factura">
                                            {{ $facturaCompras->numero_factura }}
                                        </td>
                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 category">
                                            <span
                                                class="category px-2.5 py-0.5 text-xs inline-block font-medium rounded border bg-slate-100 border-slate-200 text-slate-500 dark:bg-slate-500/20 dark:border-slate-500/20 dark:text-zink-200">{{ $factura->fecha_factura }}</span>
                                        </td>
                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 price">
                                            {{ $facturaCompras->envio_id }}</td>
                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 stock">
                                            {{ $facturaCompras->total_factura }}</td>
                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 revenue">
                                            {{ $facturaCompras->proveedor }}</td>
                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 status">
                                            {{ $factura->created_at }}</td>
                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 status">
                                            {{ $facturaCompras->updated_at }}</td>
                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zinc-500 action">
                                            <a href="{{ route('factura-compra.show', $factura->id) }}"
                                                class="text-custom-500 hover:text-custom-600"><i data-lucide="eye"
                                                    class="w-5 h-5"></i></a>
                                            <a href="{{ route('factura-compra.edit', $factura->id) }}"
                                                class="text-custom-500 hover:text-custom-600"><i data-lucide="pencil"
                                                    class="w-5 h-5"></i></a>
                                            <a href="{{ route('factura-compra.destroy', $factura->id) }}"
                                                class="text-custom-500 hover:text-custom-600"><i data-lucide="trash"
                                                    class="w-5 h-5"></i a>
                                        </td>
                                    </tr>
                                </tbody>
