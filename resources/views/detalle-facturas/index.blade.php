    ...
    <div
        ...
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            ...
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">Lista de Detalles Facturas</h5>
                </div>
            ...
            <div class="card" id="productListTable">
                <div class="card-body">
                    ...
                    <div class="lg:col-span-2 ltr:lg:text-right rtl:lg:text-left xl:col-span-2 xl:col-start-11">
                            <a href="{{ route('detalle-facturas.create') }}" type="button"
                                ...
                                <tbody class="list">
                                    <tr>
                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500">
                                            <a href="#!"
                                                class="transition-all duration-150 ease-linear ID Factura text-custom-500 hover:text-custom-600">{{ $detalleFactura->id }}</a>
                                        </td>
                                        <td
                                        ...
                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 category">
                                            <span
                                                class="category px-2.5 py-0.5 text-xs inline-block font-medium rounded border bg-slate-100 border-slate-200 text-slate-500 dark:bg-slate-500/20 dark:border-slate-500/20 dark:text-zink-200">{{ $detalleFactura
                                                ->medicamento_id }}</span>
                                        </td>
                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 price">
                                            {{ $detalleFactura
                                            ->categoria_medicamento_id }}</td>
                                        ...
                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 status">
                                         ...
                                         <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 status">
                                                {{ $detalleFactura
                                                ->fecha_vencimiento }}</td>
                                         <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zink-500 status">
                                                    {{ $detalleFactura
                                                    ->updated_at }}</td>
                                        <td class="px-3.5 py-2.5 border-y border-slate-200 dark:border-zinc-500 action">                                            
                                            <a href="{{ route('detalle-facturas.edit', $detalleFactura
                                            ->id) }}"
                                                class="text-custom-500 hover:text-custom-600"><i data-lucide="pencil"
                                                    class="w-5 h-5"></i></a>
                                            <form action="{{ route('detalle-facturas.destroy', $detalleFactura
                                            ->id) }}" method="POST" class="inline-block">
                                                @csrf
                                                @method('DELETE')
